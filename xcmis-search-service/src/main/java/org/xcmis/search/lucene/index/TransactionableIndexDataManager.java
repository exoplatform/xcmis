...
public class TransactionableIndexDataManager extends CacheableIndexDataManager
{
      ...
      IndexTransactionException
   {
      ...
      final boolean removeTransaction =
         (changes.getRemovedDocuments().size() > 0 || changes.getUpdatedDocuments().size() > 0)
            && changes.getAddedDocuments().size() == 0;

      final LoggedIndexTransactionImpl loggedIndexTransaction =
         new LoggedIndexTransactionImpl(changes.getAddedDocuments(), changes.getUpdatedDocuments(), changes
            .getRemovedDocuments(), this.transactionService);
   ...
   public void start()
   {
      ...
      try
      {
         if (this.transactionService.hasUncommitedTransactions())
         {
            ...
            compromisedUuids.addAll(compositeTransactionLog.getUpdatedList());
