...
public class ChoiceBooleanElement extends ChoiceElement<Choice<Boolean>>
{
   ...
   public void build(Choice<Boolean> choice)
   {
      if (choice != null)
      {
         ...
         if (choice.getChoices() != null && choice.getChoices().size() > 0)
         {
            ...
            {
               ChoiceBooleanElement el = addExtension(AtomCMIS.CHOICE_BOOLEAN);
               el.build(ch);
            }
         }
      }
   ...
   public Choice<Boolean> getChoice()
   {
      ...
      // CHOICE
      List<ChoiceBooleanElement> choices = getExtensions(AtomCMIS.CHOICE_BOOLEAN);
      if (choices != null && choices.size() > 0)
         for (ChoiceBooleanElement choiceBooleanElement : choices)
         {
