 ...
 * @version $Id: LeftJoinCollector.java 2 2010-02-04 17:21:49Z andrew00x $
 */
public class LeftJoinCollector implements JoinCollector
{
   ...
   public LeftJoinCollector(JoinCondition joinCondition, DocumentMatcher documentMatcher)
   {
      ...
      RepositoryException
   {
      ...
      {
         log.info("Before LeftJoinCollector");
      ...
      for (ScoredRow leftScoredNode : leftSource)
      {
         ...
         if (!isMatchFound)
         {
            result.add(leftScoredNode);
         }

      }
      if (log.isDebugEnabled())
      {
         log.info("After LeftJoinCollector");
